...
public class DBConnect {
    ...
    public void addNewRecord(double lon,double lat,String name){
        open();
    ...
    public ArrayList<MainPhotoModels> getAllRecord(){
        ...
        Cursor cursor = database.query(DBHelper.PHOTOINFO,
                new String[]{"id","create_date","create_time",
                        "file_img1","file_img2","file_img3","lon","lat","url_gm","name_record"},
        ...
        while (cursor.moveToNext()){
            rec.add(new MainPhotoModels(
                    ...
                    cursor.getString(cursor.getColumnIndex("name_record"))
            ));
        }
        close();
        return rec;
